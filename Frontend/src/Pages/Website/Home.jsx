...
const Home = () => {
  ...
  if (activecate != undefined) {
    displayproduct = product.filter((prod) => {
      ...
      {category ? (
        <div>
          ...
          <ul className="font-bold md:flex justify-center  text-base hidden gap-[10px] my-5">
            <li
              onClick={() => setActiveCate(category._id)}
              className={`cursor-pointer ${
                activecate == category._id
          ...
          <div className="items-center px-4 mb-4 md:hidden">
            <select
              id="countries"
              className="bg-gray-50 text-center  border mt-6  border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500  w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            >
          ...
          <div className="container mx-auto  pt-4 w-full justify-center lg:justify-between  md:gap-[60px] gap-[40px] flex flex-wrap ">
            {displayproduct.map((prodt, index) => {
              return (
                <div
                  key={index}
                  className=" shadow-lg truncate  relative group sm:flex lg:justify-between gap-2 py-8 sm:flex-col sm:items-center"
                >
                  ...
                  <h1 className="text-red-600  text-center">
                    ${prodt.final} <del>${prodt.price}</del>
                  </h1>
                  <Link to={`/Store/product/${prodt.slug}`}>
                    <div className=" absolute top-[100%]  left-[55px] group-hover:top-[70%]   duration-300  left-130px">
                      <button
                        type="button"
                        className="text-white  bg-[#007D88] hover:bg-[#007D88] focus:outline-none  font-medium rounded-full text-sm px-5 py-2.5  mb-2 dark:hover:bg-[#007D88] dark:focus:ring-[#007D88] "
                      >
                        Buy now
                      </button>
                    </div>
                  ...
                  <div
                    className="absolute hidden group-hover:block bg-[#007d8826] top-[0%] right-[5%] 
                                      rounded-full    duration-300 left-130px
                                    "
                  >
                    <button
                      onClick={() => dispatch(addTocart({ pId: prodt._id }))}
                      className="p-2 rounded-full  hover:bg-[#007d8826]"
                    >
                      <FaCartPlus />
                    </button>
                  </div>
          ...
          <div className="container text-center py-14 ">
            <span className=" text-indigo-700 font-bold border-solid border-b-4 border-indigo-700 ">
              LOAD MORE
            </span>
          </div>
          {/* ... your banner code ... */}
          <div className="mt-20  md:mt-10 w-full h-[600px] flex flex-wrap relative banner2">
            <div className="text-white md:ms-32   mt-12  absolute">
              <h1 className="text-4xl md:text-6xl sm:mt-2 text-center">
                iphone 6 plus
              </h1>
